...
import { TULI_MEDIA_BY_OWNER } from "data/queries"; // Query utils
...
const generateHTML = (media, address) => {
  ...
  <html>
    ...
    <div>
      <img class="tuli" src="https://r.tuli.work/logo_tuli.png" alt="Tuli" />
      <h2 class="address">${
...
export default async (req, res) => {
  ...
  const allPosts = await client.request(
    // By address
    TULI_MEDIA_BY_OWNER(address.toLowerCase())
  );
  ...
  if (ownedMedia.length > 0) {
    ...
    const image = Buffer.from(
      getBase64("https://r.tuli.work/meta.png"),
      "base64"
    );
